	...
	public static org.omg.CORBA.Any[] extract(final org.omg.CORBA.Any any) {
		if (any.type().kind() == org.omg.CORBA.TCKind.tk_null) {
			throw new org.omg.CORBA.BAD_OPERATION(
					"Can't extract from Any with null type.");
		}
			...
			synchronized (SensorSetHelper.class) {
				if (_type == null) {
					_type = org.omg.CORBA.ORB
							.init()
							.create_alias_tc(fr.upem.robot.SensorSetHelper.id(), "SensorSet", org.omg.CORBA.ORB.init()
                                                                                                               .create_sequence_tc(
                                                                                                                       0,
                                                                                                                       org.omg.CORBA.ORB
                                                                                                                               .init()
		...
		try {
			...
			if (x > 0 && _l_result0 > x) {
				throw new org.omg.CORBA.MARSHAL(
						"Sequence length too large. Only " + x
								+ " available and trying to assign "
								+ _l_result0);
			}
